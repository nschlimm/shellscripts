...
function erstelleMenu(){
   keyfunktionsmap=()
   importantLog "Alle Menuitems ausgeben"
   counter=0
   for i in "${menus[@]}"
   do
...
menus+=('01:Pulling:')
menus+=('02:Other usefulls actions:')
menus+=('03:Git admin actions:')

menuitems=()
menuitems+=('00:a. Push actual (fetch, merge, commit, push)#pushActual')
menuitems+=('00:b. Merge actual from actual origin, merge development branch to actual, push actual to origin#mergeActual')
menuitems+=('01:b. Pull master (checkout, merge, push)#pullMaster')
